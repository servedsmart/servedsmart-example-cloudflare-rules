...
JSON_REDIRECT_RULESET="$(
    cat <<EOF | jq -c
{
  "name": "${REDIRECT_RULESET_UUID_SHORT}",
  "description": "Generated by ${REPO_URL}",
  "kind": "root",
  "phase": "http_request_redirect",
  "rules": [
    {
      "expression": "http.request.full_uri in \$${REDIRECT_LIST_UUID_SHORT}",
...
RULES_LIST_ID="$(jq -r '.result[] | select(.kind == "redirect") | select(.name == "'"${REDIRECT_LIST_UUID_SHORT}"'") | .id' <<<"${API_LIST_RULES_LIST_ACCOUNT}")"
...
RULESET_ID="$(jq -r '.result[] | select(.phase == "http_request_redirect") | select(.name == "'"${REDIRECT_RULESET_UUID_SHORT}"'") | .id' <<<"${API_LIST_RULESETS_ACCOUNT}")"
if [[ -n "${RULESET_ID}" ]]; then
